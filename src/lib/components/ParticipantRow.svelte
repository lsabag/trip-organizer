<script lang="ts">
  ...
  async function removePax() {
    const idx = trip.participants.findIndex((x) => x.id === participant.id);
    if (idx < 0) return;
    // Unassign anyone assigned to this participant (if driver)
    trip.participants.forEach((p) => {
      if (p.assignedTo === participant.id) p.assignedTo = null;
    });
    const pName = trip.participants[idx].name;
